...
## September 3, 2026 01:14 PM (PHT) — prans
- **Order progress timeline** (customer `OrderTracking`): added a vertical, mobile-friendly **Order Progress** timeline — Order Placed → Received → In Queue → Printing → Completed → Released (Ready for Pickup) — with the current stage highlighted and a "Current" badge plus the order's date/time at that stage. Special states render a clear notice instead of a misleading progress bar: Canceled (red), Awaiting Payment (amber), On Hold (orange). Lets customers see where their order sits at a glance on a phone.
- **Order tables → responsive cards on mobile** (customer `CustomerDashboard` Recent Orders + `CustomerOrders` My Orders): the desktop tables are kept as-is on `sm+`, but on phones the rows now render as tappable **card rows** — order ID + file name, status badge, total, and date (with a chevron / full-width tap area). My Orders also gained a mobile empty state with a **Start New Order** button.
- **Prominent "Start a New Order" action on the customer dashboard**: the top of the homepage now leads with "Hello, [name] 👋" plus a large **Start a New Order** primary button (immediately visible without scrolling), with a compact Shop Location button beside it — prioritizing the customer's main task.
- **Order-entry navigation buttons fixed for small screens** (customer `NewPrintRequest`): the Bottom Back/Next (Cancel/Place Order) buttons were changed from fixed `min-w-[155px]` side-by-side buttons (which could overflow at 320px) to a fluid two-column grid that scales down gracefully and keeps large tap targets.
- Build passes.
