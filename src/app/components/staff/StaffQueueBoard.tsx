...
import { Kanban, LayoutDashboard, ShoppingCart, User, Search, Clock, CheckCircle, XCircle, ArrowLeft, ChevronDown, ChevronUp, Printer, FileText, AlertCircle, AlertTriangle, CreditCard, Package, LayoutGrid, Boxes, Users, UserPlus, Briefcase, File, Eye } from 'lucide-react';
...
export default function StaffQueueBoard() {
  ...
  return (
    <Layout menuItems={menuItems} title="Orders" showBackButton>
                ...
                {selectedOrder.attachedFiles && selectedOrder.attachedFiles.length > 0 && (
                  <div className="p-4 bg-white border border-gray-200 rounded-xl">
                    <div className="flex items-center justify-between mb-3">
                      <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Attached Files</p>
                      <Button
                        size="sm"
                        variant="outline"
                        className="h-7 text-xs gap-1.5 border-[#1D73EC] text-[#1D73EC] hover:bg-blue-50"
      ...
      {/* Print Preview Dialog — Staff Inspect */}
      {selectedOrder && (
        <PrintPreviewDialog
          open={showPrintPreview}
          onOpenChange={setShowPrintPreview}
          fileName={selectedOrder.attachedFiles?.[0]?.name ?? 'document.pdf'}
          pageCount={selectedOrder.pages ?? 1}
          userRole="staff"
          orderId={selectedOrder.id}
          options={{
            paperSize: selectedOrder.paperSize?.toLowerCase() ?? 'a4',
            printType: selectedOrder.colorMode === 'color' || selectedOrder.colorMode === 'colored' ? 'Colored' : 'Black & White',
            copies: selectedOrder.copies ?? 1,
            orientation: (selectedOrder.orientation as 'portrait' | 'landscape') ?? 'portrait',
            pagesPerSheet: selectedOrder.pagesPerSheet ?? '1',
            twoSided: selectedOrder.twoSided ?? 'no',
            pageRange: selectedOrder.pageRange ?? 'all',
            specificPages: (selectedOrder as any).specificPages,
          }}
        />
      )}
