    ...
    >
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <Card className="p-6 bg-white shadow-sm">
            ...
            <div className="mb-6">
              <div className="flex items-center justify-between mb-3">
                ...
                <Button
                  size="sm"
                  variant="outline"
                  className="h-8 text-xs gap-1.5 border-[#2F6FD6] text-[#2F6FD6] hover:bg-blue-50"
          ...
          {/* Print Preview — Customer read-only */}
          {orderData && (
            <PrintPreviewDialog
              open={showPrintPreview}
              onOpenChange={setShowPrintPreview}
              fileName={orderData.attachedFiles?.[0]?.name ?? orderData.fileName ?? "document.pdf"}
              pageCount={orderData.pages ?? 1}
              userRole="customer"
              orderId={orderData.id}
              readOnly
              options={{
                paperSize: orderData.paperSize?.toLowerCase() ?? "a4",
                printType: orderData.colorMode === "colored" || orderData.colorMode === "color" ? "Colored" : orderData.printType ?? "Black & White",
                copies: orderData.copies ?? 1,
                orientation: (orderData.orientation as "portrait" | "landscape") ?? "portrait",
                pagesPerSheet: orderData.pagesPerSheet ?? "1",
                twoSided: orderData.twoSided ?? "no",
